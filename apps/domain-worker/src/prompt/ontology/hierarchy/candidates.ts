...
import type { TypedHierarchyEdge } from "./types.js";
...
export type HierarchyEvidenceSignal =
  (typeof HIERARCHY_EVIDENCE_SIGNALS)[number];
...
export function hierarchyEvidenceFromStructuralContext(
  ...
  return [
    ...
    ...(context.repeatedParentBlocks?.length
      ? ["repeated_parent_blocks" as const]
      : []),
  ...
  return inputs
    .map((input) => {
      ...
      return {
        ...
        evidence,
        confidence: Number(
          (evidence.length / HIERARCHY_EVIDENCE_SIGNALS.length).toFixed(6),
        ),
        reviewStatus: "proposed" as const,
      };
    })
    .sort((left, right) =>
      [left.hierarchyId, left.hierarchyVersion, left.sourceId, left.targetId]
        .join("\u001f")
        .localeCompare(
          [
            right.hierarchyId,
            right.hierarchyVersion,
            right.sourceId,
            right.targetId,
          ].join("\u001f"),
        ),
