...
function isWithinHierarchyInterval(
  ...
  return (
    (!hierarchy.validFrom ||
      !edge.validFrom ||
      hierarchy.validFrom <= edge.validFrom) &&
    (!hierarchy.validTo ||
      !edge.validTo ||
      edge.validTo <= hierarchy.validTo) &&
    (!hierarchy.validFrom ||
      !edge.validTo ||
      hierarchy.validFrom <= edge.validTo) &&
    (!hierarchy.validTo ||
      !edge.validFrom ||
      edge.validFrom <= hierarchy.validTo)
  );
}
...
function coverageStatus(
  ...
  if (!hierarchy.coverage)
    return { status: "unassessed", missingMemberIds: [] };
  ...
  if (missingMemberIds.length === 0)
    return { status: "complete", missingMemberIds };
  return {
...
export function validateTypedHierarchy(input: {
  ...
  const canonicalValues = new Map(
    input.canonicalValues.map((value) => [value.id, value]),
  );
  ...
  for (const level of hierarchy.levels) {
    if (levelIds.has(level.id) || levelOrders.has(level.order)) {
      add({
        code: "unknown_level",
        message: "Hierarchy level IDs and orders must be unique.",
      });
    }
  ...
  const activeRollups = activeEdges.filter(
    (edge) => edge.relation === "rolls_up_to",
  );
  if (
    ...
    add({
      ...
      message:
        "Active calendar and fiscal roll-ups require an explicit period scheme ID and version.",
    });
  }
  if (
    (hierarchy.type === "calendar" || hierarchy.type === "fiscal") &&
    ...
    add({
      ...
      message:
        "A calendar/fiscal hierarchy period scheme kind must match the hierarchy type.",
    });
  }
  const periodScheme = hierarchy.periodScheme;
  if (periodScheme) {
    const metadata = input.periodSchemeMetadata?.find((candidate) =>
      sameScheme(candidate.valueScheme, periodScheme),
    );
    if (metadata && metadata.periodSchemeKind !== hierarchy.periodSchemeKind) {
      add({
        code: "period_scheme_kind",
        message:
          "Hierarchy period scheme kind must match registered value-scheme metadata.",
      });
    }
  }
  if (!isIntervalValid(hierarchy.validFrom, hierarchy.validTo)) {
    add({
      ...
      message:
        "Hierarchy validity interval must contain real dates and must not end before it starts.",
    });
  }
  ...
  for (const requiredMemberId of hierarchy.coverage?.requiredMemberIds ?? []) {
    ...
    if (!member) {
      add({
        code: "unknown_value",
        message: `Hierarchy coverage references an unknown canonical member: ${requiredMemberId}.`,
      });
      continue;
    }
    const levelSchemes = hierarchy.levels.flatMap((level) =>
      level.valueScheme ? [level.valueScheme] : [],
    );
    ...
    if (!inScope) {
      add({
        ...
        message: `Hierarchy coverage member is outside the declared level schemes: ${requiredMemberId}.`,
      });
    }
  }
  ...
  hierarchy.edges.forEach((edge, edgeIndex) => {
    if (
      !HIERARCHY_EDGE_RELATIONS.includes(
        edge.relation as (typeof HIERARCHY_EDGE_RELATIONS)[number],
      )
    ) {
      add({
        code: "invalid_relation",
        edgeIndex,
        message: "Hierarchy edge has an unsupported relation.",
      });
      return;
    }
    if (
      ...
      add({
        code: "hierarchy_identity",
        edgeIndex,
        message:
          "Edge hierarchy ID and version must match the containing hierarchy.",
      });
    }
    ...
    if (edgeKeys.has(edgeKey)) {
      add({
        code: "duplicate_edge",
        edgeIndex,
        message: "Duplicate typed hierarchy edge.",
      });
    }
    edgeKeys.add(edgeKey);
    if (
      !isIntervalValid(edge.validFrom, edge.validTo) ||
      !isWithinHierarchyInterval(edge, hierarchy)
    ) {
      add({
        code: "invalid_interval",
        edgeIndex,
        message:
          "Edge validity interval contains an impossible date, is reversed, or falls outside its hierarchy interval.",
      });
    }
    if (edge.relation !== "rolls_up_to") {
      if (
        ...
        add({
          ...
          edgeIndex,
          message:
            "Only statistical roll-up edges may carry level or aggregation metadata.",
        });
      }
      return;
    }
    ...
    if (!source || !target) {
      add({
        code: "unknown_value",
        edgeIndex,
        message: "Roll-up edges must connect declared canonical values.",
      });
    }
    const sourceLevel = edge.sourceLevelId
      ? levels.get(edge.sourceLevelId)
      : undefined;
    const targetLevel = edge.targetLevelId
      ? levels.get(edge.targetLevelId)
      : undefined;
    if (!sourceLevel || !targetLevel) {
      add({
        code: "unknown_level",
        edgeIndex,
        message: "Roll-up edges must name declared source and target levels.",
      });
    }
    ...
    const isAdditive =
      edge.aggregation === "sum" || edge.aggregation === "weighted_mean";
    if (
      source?.valueRole === "total" ||
      ...
      if (
        !aggregation ||
        !sameScheme(
          aggregation.sourceValueScheme,
          aggregation.targetValueScheme,
        ) ||
        !sameScheme(
          aggregation.sourceValueScheme,
          source?.valueScheme ?? aggregation.sourceValueScheme,
        ) ||
        !sameScheme(
          aggregation.targetValueScheme,
          target?.valueScheme ?? aggregation.targetValueScheme,
        ) ||
        aggregation.sourceUniverse !== aggregation.targetUniverse ||
        aggregation.sourceAggregationRule !==
          aggregation.targetAggregationRule ||
        !["sum", "weighted_mean"].includes(aggregation.sourceAggregationRule) ||
        edge.aggregation !== aggregation.sourceAggregationRule
      ) {
        add({
          code: "incompatible_aggregation",
          edgeIndex,
          message:
            "Additive and totals/member roll-ups require compatible universe, scheme version, and matching additive aggregation rules.",
        });
      }
    }
    ...
    if (sourceLevel && targetLevel) {
      ...
      if (sourceLevel.order !== targetLevel.order + 1) {
        add({
          ...
          edgeIndex,
          message: "Roll-up levels must be adjacent child-to-parent levels.",
        });
      }
      if (
        source &&
        sourceLevel.valueScheme &&
        !sameScheme(source.valueScheme, sourceLevel.valueScheme)
      ) {
        add({
          code: "scheme_mismatch",
          edgeIndex,
          message:
            "Roll-up source does not match its level value-scheme version.",
        });
      }
      if (
        target &&
        targetLevel.valueScheme &&
        !sameScheme(target.valueScheme, targetLevel.valueScheme)
      ) {
        add({
          code: "scheme_mismatch",
          edgeIndex,
          message:
            "Roll-up target does not match its level value-scheme version.",
        });
      }
      const temporalUniqueParent =
        hierarchy.type === "calendar" || hierarchy.type === "fiscal";
      if (sourceLevel.requiresUniqueParent || temporalUniqueParent) {
        ...
        if (parentKeys.has(parentKey)) {
          add({
            ...
            edgeIndex,
            message:
              "This hierarchy level requires one roll-up parent per member.",
          });
        }
    ...
    if (periodScheme) {
      if (source && !sameScheme(source.valueScheme, periodScheme)) {
        add({
          ...
          edgeIndex,
          message:
            "Temporal roll-up source must use the hierarchy period scheme version.",
        });
      }
      if (target && !sameScheme(target.valueScheme, periodScheme)) {
        add({
          ...
          edgeIndex,
          message:
            "Temporal roll-up target must use the hierarchy period scheme version.",
        });
      }
    }
  });
  ...
  if (hasDirectedCycle(activeEdges)) {
    add({
      code: "cycle",
      message: "Active typed hierarchy edges must be acyclic.",
    });
  }
  const coverage = coverageStatus(hierarchy, activeRollups);
  if (coverage.status === "orphaned") {
    add({
      code: "orphaned_member",
      message: `Required hierarchy members lack active parents: ${coverage.missingMemberIds.join(", ")}.`,
    });
  }
  issues.sort((left, right) =>
    `${left.code}\u001f${left.edgeIndex ?? -1}\u001f${left.message}`.localeCompare(
      `${right.code}\u001f${right.edgeIndex ?? -1}\u001f${right.message}`,
    ),
  );
