...
import { compatibilityWarningsForOccurrence } from "./publicationCompatibility.js";
import {
  safeParsePublicationOntology,
  type PublicationOntology,
} from "./publicationSchema.js";
import type { OntologyDetection, SheetOntology } from "./types.js";
import type { OntologyOverrides, ResolvedSheetOntology } from "./overrides.js";
...
export const PUBLICATION_ONTOLOGY_TOKEN_ESTIMATOR =
  "utf16_chars_div_4_ceil_v1" as const;
...
export function buildOntologyPromptPayload(
  ...
  if (feature.artifact === undefined) {
    provenance.artifact.reason =
      feature.unavailableReason ?? "Publication artifact was not supplied.";
  ...
  if (!parsed.success) {
    provenance.artifact.reason =
      "Publication artifact failed strict validation.";
  ...
  if (
    ...
    provenance.artifact.reason =
      "Publication prompt scope or independent pin is incomplete.";
  ...
  if (!samePin(feature.expectedPin, actualPin(artifact))) {
    provenance.artifact.reason =
      "Publication artifact does not match the independent expected pin.";
  ...
  if (!isReviewed(artifact.profile.status)) {
    provenance.artifact.reason =
      "Publication profile is not reviewed or published.";
  ...
  const hierarchyHints = promptHierarchyHints(
    artifact,
    approvedBindings,
    omissionCounts,
  );
  ...
  let used =
    PUBLICATION_ONTOLOGY_PROMPT_GUIDANCE.join("\n").length +
  ...
  const admit = <T>(
    section: keyof Pick<
      ...
      "hardExclusions" | "approvedBindings" | "approvedHierarchies"
    >,
    item: T,
  ) => {
  ...
  hardExclusions
    ...
    .forEach((item) => admit("hardExclusions", item));
  approvedBindings
    .sort(bindingPriorityCompare)
    .forEach((item) => admit("approvedBindings", item));
  hierarchyHints
    .sort(stableEntityCompare)
    .forEach((item) => admit("approvedHierarchies", item));
  ...
  provenance.legacyFallback =
    budgetedLegacy.length > 0 ? "used" : "unavailable";
  ...
  if (!hasPublicationContent) {
    provenance.artifact.reason =
      "No current approved publication guidance matched the requested scope.";
    return fallbackPayload(originalLegacy, provenance);
  }
...
function approvedPromptBindings({
  ...
  for (const occurrence of [...artifact.occurrenceMappings].sort((a, b) =>
    a.occurrenceId.localeCompare(b.occurrenceId),
  )) {
    if (
      !("rawDetectionIds" in occurrence) ||
      !("structuralEvidence" in occurrence)
    )
      continue;
    if (
    ...
    const decisions = artifact.occurrenceVariableBindings.filter(
      (binding) => binding.occurrenceId === occurrence.occurrenceId,
    );
    ...
    if (decisions.length > 0 && approvedDecisions.length !== 1) {
      increment(
        omissions,
        approvedDecisions.length > 1 ? "ambiguous" : "not_approved",
      );
      continue;
    }
    const explicit = approvedDecisions[0];
    const represented = artifact.representedVariables.find(
      (item) => item.id === occurrence.representedVariableId,
    );
    if (!represented || !isReviewed(represented.status)) {
      increment(omissions, "stale");
      continue;
    }
    const conceptual = artifact.conceptualVariables.find(
      (item) => item.id === represented.conceptualVariableId,
    );
    ...
    if (
      ...
      (!represented.valueScheme ||
        explicit.valueScheme.valueSchemeId !==
          represented.valueScheme.valueSchemeId ||
        explicit.valueScheme.valueSchemeVersion !==
          represented.valueScheme.valueSchemeVersion)
    ) {
      increment(omissions, "version_conflict");
      continue;
    }
        ...
        (ontology) =>
          ...
          ontology.detections.some(
            (detection) =>
              ...
              sameStrings(
                detection.addresses,
                occurrence.sourceLocation.addresses,
              ) &&
              detection.joinKey,
          ),
      ),
    ...
    results.push({
      ...
      concept: {
        id: concept.id,
        label: preferredLabel(concept.preferredLabels),
      },
      representedVariable: {
        id: represented.id,
      ...
      valueScheme: represented.valueScheme
        ? {
            id: represented.valueScheme.valueSchemeId,
            version: represented.valueScheme.valueSchemeVersion,
          }
        : null,
      joinKey,
...
function promptHardExclusions({
  ...
  for (const binding of artifact.occurrenceVariableBindings) {
    if (binding.reviewStatus !== "rejected" || !binding.representedVariableId)
      continue;
    const occurrence = artifact.occurrenceMappings.find(
      (item) => item.occurrenceId === binding.occurrenceId,
    );
    if (
      !occurrence ||
      !("rawDetectionIds" in occurrence) ||
    ...
    const represented = artifact.representedVariables.find(
      (item) => item.id === binding.representedVariableId,
    );
    const conceptual = represented
      ? artifact.conceptualVariables.find(
          (item) => item.id === represented.conceptualVariableId,
        )
      : undefined;
    const concept = conceptual
    ...
    if (
      ...
      (binding.valueScheme &&
        (!represented.valueScheme ||
          binding.valueScheme.valueSchemeId !==
            represented.valueScheme.valueSchemeId ||
          binding.valueScheme.valueSchemeVersion !==
            represented.valueScheme.valueSchemeVersion))
    ) {
      increment(omissions, "version_conflict");
      continue;
  ...
  const relevantSchemeVersions = new Set(
    approvedBindings.flatMap((item) =>
      item.valueScheme
        ? [`${item.valueScheme.id}@${item.valueScheme.version}`]
        : [],
    ),
  );
  for (const hierarchy of artifact.hierarchies) {
    ...
    const currentLevels = hierarchy.levels.filter((level) => {
      ...
      return artifact.valueSchemes.some(
        (item) =>
    ...
    for (const edge of hierarchy.edges) {
      ...
      if (
        !levelIds.has(edge.sourceLevelId) ||
        !levelIds.has(edge.targetLevelId)
      )
        continue;
      exclusions.push({
...
function promptHierarchyHints(
  ...
  const relevantSchemeIds = new Set(
    bindings.flatMap((item) => (item.valueScheme ? [item.valueScheme.id] : [])),
  );
  const relevantSchemeVersions = new Set(
  ...
  return artifact.hierarchies.flatMap((hierarchy) => {
    const candidateLevels = hierarchy.levels.filter(
      (level) =>
        level.valueScheme &&
        relevantSchemeIds.has(level.valueScheme.valueSchemeId),
    );
    ...
    const hasVersionConflict = candidateLevels.some((level) => {
      if (!level.valueScheme) return false;
      const key = `${level.valueScheme.valueSchemeId}@${level.valueScheme.valueSchemeVersion}`;
      const scheme = artifact.valueSchemes.find(
        (item) =>
          item.id === level.valueScheme?.valueSchemeId &&
          item.version === level.valueScheme?.valueSchemeVersion,
      );
      return (
        !relevantSchemeVersions.has(key) ||
        !scheme ||
        !isReviewed(scheme.status)
      );
    });
    const periodScheme = hierarchy.periodScheme
      ? artifact.valueSchemes.find(
    ...
    if (
      ...
      (hierarchy.periodScheme &&
        (!periodScheme || !isReviewed(periodScheme.status)))
    ) {
      increment(omissions, "version_conflict");
      return [];
    }
    const levels = candidateLevels
      .map((level) => ({
        ...
        valueScheme: level.valueScheme
          ? {
              id: level.valueScheme.valueSchemeId,
              version: level.valueScheme.valueSchemeVersion,
            }
          : null,
      }))
    ...
    const approvedRelations = hierarchy.edges
      .filter((edge) => {
        ...
        return (
          Boolean(edge.sourceLevelId && levelIds.has(edge.sourceLevelId)) &&
...
function baseProvenance(
  ...
  return {
    ...
    budget: {
      ...
      maxEstimatedTokens: estimatedTokens(
        PUBLICATION_ONTOLOGY_PROMPT_CHAR_BUDGET,
      ),
      usedCharacters: 0,
      estimatedTokens: 0,
...
function fallbackPayload(
  ...
  provenance.included =
    legacy.length > 0 ? { legacySheetOntology: legacy.length } : {};
...
function samePin(
  left: PublicationOntologyPromptPin,
  right: PublicationOntologyPromptPin,
): boolean {
  return (
...
function serializeOntology(ontology: OntologyPromptInput) {
  ...
  return {
    ...
    activeDetections: activeDetections.map((detection) =>
      compactDetection(
        detection,
        promptSourceForDetection(detection, overrides),
...
function preferredLabel(
  labels: Array<{ language: string; value: string }>,
): string {
  return (
    labels.find((label) => label.language.toLowerCase().startsWith("en"))
      ?.value ??
    labels[0]?.value ??
    ""
  );
...
function bindingPriorityCompare(
  left: ApprovedBinding,
  right: ApprovedBinding,
): number {
  const priority = (item: ApprovedBinding) =>
    item.bindingMethod === "publication_override"
      ? 0
      : item.joinKey || item.temporal
        ? 1
        : 2;
  return (
