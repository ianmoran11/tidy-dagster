...
} from "./publicationIds.js";
import { createOccurrenceStructuralSignature } from "./occurrences.js";
import {
...
const semanticVersionSchema = z
  .string()
  .regex(SEMVER_PATTERN, "Expected semantic version.");
...
const genericIdSchema = z
  .string()
  .regex(GENERIC_ID_PATTERN, "Expected stable typed ID.");
export const ExternalConceptSchemeIdSchema = z
  .string()
  .regex(
    CONCEPT_SCHEME_ID_PATTERN,
    "Expected stable external concept-scheme ID.",
  );
...
function entityIdSchema(prefix: PublicationIdPrefix) {
  return z
    .string()
    .refine(
      (value) => isPublicationEntityId(value, prefix),
      `Expected stable ${prefix} ID.`,
    );
}
...
export const OBSERVATION_STATUS_ROLES = [
  "none",
  "attribute",
  "flag",
  "suppression",
...
const crosswalkRelationSchema = z.enum([
  "exact",
  "close",
  "broader",
  "narrower",
    ...
    for (const label of labels) {
      ...
      if (duplicates.has(key)) {
        context.addIssue({
          code: "custom",
          message:
            "Labels must not be duplicated across preferred, alternative, or hidden labels.",
        });
        return;
      }
      duplicates.add(key);
...
export const HierarchyEdgeSchema = z
  .strictObject({
    ...
    aggregation: z
      .enum(["sum", "weighted_mean", "non_additive", "unknown"])
      .optional(),
    aggregationContext: z
      .strictObject({
  ...
  .superRefine((edge, context) => {
    if (edge.relation === "rolls_up_to") {
      if (!edge.sourceLevelId) {
        addIssue(
          context,
          ["sourceLevelId"],
          "Roll-up edges require a source level ID.",
        );
      }
      if (!edge.targetLevelId) {
        addIssue(
          context,
          ["targetLevelId"],
          "Roll-up edges require a target level ID.",
        );
      }
    } else if (
      edge.sourceLevelId ||
    ...
    if (edge.validFrom && edge.validTo && edge.validFrom > edge.validTo) {
      addIssue(
        context,
        [],
        "Hierarchy edge validity interval must not end before it starts.",
      );
    }
  });
...
export const DurableOccurrenceMappingSchema = z
  .strictObject({
    ...
    structuralSignature: z
      .string()
      .regex(/^occurrence_signature_v\d+_[a-z0-9]{8,32}$/),
  ...
  .superRefine((occurrence, context) => {
    if (
      new Set(occurrence.rawDetectionIds).size !==
      occurrence.rawDetectionIds.length
    ) {
      addIssue(
        context,
        ["rawDetectionIds"],
        "Raw detection IDs must be unique within an occurrence.",
      );
    }
    if (
      occurrence.rawDetectionIds.includes(occurrence.occurrenceId) ||
      occurrence.rawDetectionIds.some((id) =>
        isPublicationEntityId(id, "occurrence"),
      )
    ) {
      addIssue(
        context,
    ...
    if (new Set(occurrence.supersedes).size !== occurrence.supersedes.length) {
      addIssue(
        context,
        ["supersedes"],
        "Superseded occurrence IDs must be unique.",
      );
    }
    if (
      new Set(occurrence.supersededBy).size !== occurrence.supersededBy.length
    ) {
      addIssue(
        context,
        ["supersededBy"],
        "Superseding occurrence IDs must be unique.",
      );
    }
    if (occurrence.supersedes.includes(occurrence.occurrenceId)) {
      addIssue(
        context,
        ["supersedes"],
        "An occurrence cannot supersede itself.",
      );
    }
    if (occurrence.supersededBy.includes(occurrence.occurrenceId)) {
      addIssue(
        context,
        ["supersededBy"],
        "An occurrence cannot be superseded by itself.",
      );
    }
    if (
...
export const OccurrenceVariableBindingSchema = z
  ...
  .superRefine((binding, context) => {
    ...
    if (
      (hasTarget && targetReferenceCount !== 4) ||
      (!hasTarget && targetReferenceCount !== 0)
    ) {
      addIssue(
        context,
        ["representedVariableId"],
    ...
    if (binding.reviewStatus === "abstained" && hasTarget) {
      addIssue(
        context,
        ["representedVariableId"],
        "Abstained occurrence decisions cannot bind a represented variable.",
      );
    }
    if (binding.reviewStatus !== "abstained" && !hasTarget) {
      addIssue(
        context,
        ["representedVariableId"],
        "Non-abstained occurrence decisions require a represented-variable binding.",
      );
    }
    if (
      binding.reviewStatus === "approved" &&
      !["publication_override", "existing_approved_occurrence"].includes(
        binding.method,
      )
    ) {
      addIssue(
        context,
        ["method"],
        "Only a publication override or existing approved occurrence may approve a binding.",
      );
    }
  });
...
const publicationOntologyBaseSchema = z.strictObject({
  ...
  occurrenceVariableBindings: z
    .array(OccurrenceVariableBindingSchema)
    .default([]),
...
export const PublicationOntologySchema =
  publicationOntologyBaseSchema.superRefine((artifact, context) =>
    validatePublicationOntologyReferences(artifact, context),
  );
...
export type ValueSchemeCrosswalkEntry = z.infer<
  typeof ValueSchemeCrosswalkEntrySchema
>;
export type OccurrenceVariableBinding = z.infer<
  typeof OccurrenceVariableBindingSchema
>;
...
export function isApprovedCrosswalkEntry(
  entry: ValueSchemeCrosswalkEntry,
): boolean {
...
function validatePublicationOntologyReferences(
  ...
  const publisherIds = new Set<string>(
    artifact.publisherSchemes.map((scheme) => scheme.id),
  );
  ...
  if (!publisherIds.has(artifact.profile.publisherId)) {
    addIssue(
      context,
      ["profile", "publisherId"],
      "Publication profile must reference a publisher scheme.",
    );
  }
  for (const publisherSchemeId of artifact.profile.publisherSchemeIds) {
    if (!publisherIds.has(publisherSchemeId)) {
      addIssue(
        context,
        ["profile", "publisherSchemeIds"],
        "Unknown publisher scheme ID.",
      );
    }
  }
  ...
  const conceptIds = new Set<string>(
    artifact.concepts.map((concept) => concept.id),
  );
  const conceptualVariableIds = new Set<string>(
    artifact.conceptualVariables.map((variable) => variable.id),
  );
  const representedVariableIds = new Set<string>(
    artifact.representedVariables.map((variable) => variable.id),
  );
  const occurrenceIds = new Set<string>(
    artifact.occurrenceMappings.map((occurrence) => occurrence.occurrenceId),
  );
  const schemeByKey = new Map(
    artifact.valueSchemes.map((scheme) => [
      schemeKey(scheme.id, scheme.version),
      scheme,
    ]),
  );
  const canonicalValueById = new Map<
    string,
    (typeof artifact.canonicalValues)[number]
  ...
  for (const concept of artifact.concepts) {
    if (!allowedOwnerIds.has(concept.ownerSchemeId)) {
      addIssue(
        context,
        ["concepts"],
        "Concept references an unknown ontology owner.",
      );
    }
  }
  ...
  for (const valueScheme of artifact.valueSchemes) {
    if (!allowedOwnerIds.has(valueScheme.ownerId)) {
      addIssue(
        context,
        ["valueSchemes"],
        "Value scheme references an unknown ontology owner.",
      );
    }
  }
  ...
  for (const conceptualVariable of artifact.conceptualVariables) {
    if (!conceptIds.has(conceptualVariable.conceptId)) {
      addIssue(
        context,
        ["conceptualVariables"],
        "Conceptual variable references an unknown concept.",
      );
    }
  }
  ...
  for (const variable of artifact.representedVariables) {
    if (!conceptualVariableIds.has(variable.conceptualVariableId)) {
      addIssue(
        ...
        "Represented variable references an unknown conceptual variable.",
      );
    }
    if (
      variable.valueScheme &&
      !schemeByKey.has(schemeKeyOf(variable.valueScheme))
    ) {
      addIssue(
        context,
        ["representedVariables"],
        "Represented variable references an unknown value-scheme version.",
      );
    }
  }
  ...
  for (const value of artifact.canonicalValues) {
    if (!schemeByKey.has(schemeKeyOf(value.valueScheme))) {
      addIssue(
        context,
        ["canonicalValues"],
        "Canonical value references an unknown value-scheme version.",
      );
    }
    ...
    if (canonicalCodes.has(key)) {
      addIssue(
        context,
        ["canonicalValues"],
        "Canonical value codes must be unique within a scheme version.",
      );
    }
  ...
  for (const mapping of artifact.rawValueMappings) {
    if (mapping.publicationId !== artifact.profile.id) {
      addIssue(
        context,
        ["rawValueMappings"],
        "Raw-value mapping must belong to the publication profile.",
      );
    }
    ...
    if (!representedVariable) {
      addIssue(
        context,
        ["rawValueMappings"],
        "Raw-value mapping references an unknown represented variable.",
      );
    } else if (
      ...
      addIssue(
        context,
        ["rawValueMappings"],
        "Raw-value mapping value scheme must match the represented-variable definition.",
      );
    }
    if (!schemeByKey.has(schemeKeyOf(mapping.valueScheme))) {
      addIssue(
        context,
        ["rawValueMappings"],
        "Raw-value mapping references an unknown value-scheme version.",
      );
    }
    const mappingKey = [
    ...
    if (mappingKeys.has(mappingKey)) {
      addIssue(
        context,
        ["rawValueMappings"],
        "Duplicate or conflicting raw-value mapping for the same variable and scheme version.",
      );
    }
    ...
    if (mapping.relation === "unmapped" && mapping.canonicalValueId) {
      addIssue(
        context,
        ["rawValueMappings"],
        "Unmapped raw values cannot reference a canonical value.",
      );
    }
    if (mapping.relation !== "unmapped" && !mapping.canonicalValueId) {
      addIssue(
        context,
        ["rawValueMappings"],
        "Mapped raw values require a canonical value ID.",
      );
    }
    if (mapping.canonicalValueId) {
      ...
      if (!canonicalValue) {
        addIssue(
          context,
          ["rawValueMappings"],
          "Raw-value mapping references an unknown canonical value.",
        );
      } else if (!sameScheme(canonicalValue.valueScheme, mapping.valueScheme)) {
        addIssue(
          context,
          ["rawValueMappings"],
          "Canonical value must use the mapping's scheme ID and version.",
        );
      }
    }
  }
  ...
  for (const crosswalk of artifact.valueSchemeCrosswalks) {
    if (
      !schemeByKey.has(schemeKeyOf(crosswalk.sourceScheme)) ||
      !schemeByKey.has(schemeKeyOf(crosswalk.targetScheme))
    ) {
      addIssue(
        context,
        ["valueSchemeCrosswalks"],
        "Crosswalk references an unknown source or target scheme version.",
      );
    }
    for (const entry of crosswalk.entries) {
      ...
      if (!source || !target) {
        addIssue(
          context,
          ["valueSchemeCrosswalks"],
          "Crosswalk entry references an unknown canonical value.",
        );
      } else if (
        !sameScheme(source.valueScheme, crosswalk.sourceScheme) ||
        !sameScheme(target.valueScheme, crosswalk.targetScheme)
      ) {
        addIssue(
          context,
          ["valueSchemeCrosswalks"],
          "Crosswalk entry canonical values must match source and target scheme versions.",
        );
      }
    }
  }
  ...
  for (const hierarchy of artifact.hierarchies) {
    ...
    if (hierarchy.periodScheme && !periodScheme) {
      addIssue(
        context,
        ["hierarchies"],
        "Hierarchy period scheme references an unknown value-scheme version.",
      );
    }
    if (
      periodScheme &&
      ...
      addIssue(
        context,
        ["hierarchies"],
        "Hierarchy type must match its registered calendar/fiscal period scheme kind.",
      );
    }
    for (const requiredMemberId of hierarchy.coverage?.requiredMemberIds ??
      ...
      if (!canonicalValueById.has(requiredMemberId)) {
        addIssue(
          context,
          ["hierarchies"],
          "Hierarchy coverage references an unknown canonical member.",
        );
      }
    }
    ...
    const levelById = new Map(
      hierarchy.levels.map((level) => [level.id, level]),
    );
    for (const level of hierarchy.levels) {
      if (levelIds.has(level.id)) {
        addIssue(
          context,
          ["hierarchies"],
          "Hierarchy level ID must be unique.",
        );
      }
      ...
      if (levelOrders.has(level.order)) {
        addIssue(
          context,
          ["hierarchies"],
          "Hierarchy level order must be unique.",
        );
      }
      levelOrders.add(level.order);
      if (
        level.valueScheme &&
        !schemeByKey.has(schemeKeyOf(level.valueScheme))
      ) {
        addIssue(
          context,
          ["hierarchies"],
          "Hierarchy level references an unknown value-scheme version.",
        );
      }
    }
    ...
    for (const edge of hierarchy.edges) {
      if (
        edge.hierarchyId !== hierarchy.id ||
        edge.hierarchyVersion !== hierarchy.version
      ) {
        addIssue(
          context,
          ["hierarchies"],
          "Every hierarchy edge must name its containing hierarchy ID and version.",
        );
      }
      if (
        !knownEntityIds.has(edge.sourceId) ||
        !knownEntityIds.has(edge.targetId)
      ) {
        addIssue(
          context,
          ["hierarchies"],
          "Hierarchy edge references an unknown entity.",
        );
      }
      if (edge.sourceId === edge.targetId) {
        addIssue(
          context,
          ["hierarchies"],
          "Hierarchy edges cannot be self-referential.",
        );
      }
      ...
      if (edgeKeys.has(edgeKey)) {
        addIssue(
          context,
          ["hierarchies"],
          "Duplicate hierarchy edges are not allowed.",
        );
      }
      ...
      if (edge.relation === "rolls_up_to" && isActiveHierarchyEdge(edge)) {
        ...
        if (!sourceValue || !targetValue) {
          addIssue(
            context,
            ["hierarchies"],
            "Statistical roll-up edges must connect canonical values.",
          );
        }
        ...
        if (!sourceLevel || !targetLevel) {
          addIssue(
            context,
            ["hierarchies"],
            "Roll-up edges must reference known hierarchy levels.",
          );
        ...
        if (
          ...
          addIssue(
            context,
            ["hierarchies"],
            "Roll-up source value must match its level's value scheme version.",
          );
        }
        if (
          targetValue &&
          ...
          addIssue(
            context,
            ["hierarchies"],
            "Roll-up target value must match its level's value scheme version.",
          );
        }
      }

      if (
        edge.relation === "broader_concept" &&
        (!conceptIds.has(edge.sourceId) || !conceptIds.has(edge.targetId))
      ) {
        addIssue(
          context,
          ["hierarchies"],
          "Conceptual broader edges must connect concepts.",
        );
      }

      if (
        edge.relation === "contains" &&
        (edge.sourceId !== artifact.profile.id ||
          !occurrenceIds.has(edge.targetId))
      ) {
        addIssue(
          context,
          ["hierarchies"],
    ...
    if (hasDirectedCycle(hierarchy.edges.filter(isActiveHierarchyEdge))) {
      addIssue(
        context,
        ["hierarchies"],
        "Active hierarchy edges must form an acyclic directed graph.",
      );
    }
    ...
    const typedResult = validateTypedHierarchy({
      ...
      periodSchemeMetadata: artifact.valueSchemes.map((scheme) => ({
        valueScheme: {
          valueSchemeId: scheme.id,
          valueSchemeVersion: scheme.version,
        },
  ...
  const occurrenceById = new Map(
    artifact.occurrenceMappings.map((occurrence) => [
  ...
  for (const occurrence of artifact.occurrenceMappings) {
    if (occurrence.publicationId !== artifact.profile.id) {
      addIssue(
        context,
        ["occurrenceMappings"],
        "Occurrence mapping must reference this publication.",
      );
    }
    if (
      occurrence.representedVariableId &&
      !representedVariableIds.has(occurrence.representedVariableId)
    ) {
      addIssue(
        context,
        ["occurrenceMappings"],
        "Occurrence mapping references an unknown represented variable.",
      );
    }
    ...
    if (
      ...
      occurrence.structuralSignatureVersion !==
        expectedSignature.signatureVersion
    ) {
      addIssue(
        context,
        ["occurrenceMappings"],
    ...
    if (
      ...
      !schemeByKey.has(
        schemeKeyOf(occurrence.structuralEvidence.classification),
      )
    ) {
      addIssue(
        context,
    ...
    for (const supersededOccurrenceId of occurrence.supersedes) {
      ...
      if (!superseded) {
        addIssue(
          context,
          ["occurrenceMappings"],
          "Occurrence supersession references an unknown occurrence.",
        );
      } else if (
        !isDurableOccurrence(superseded) ||
        !superseded.supersededBy.includes(occurrence.occurrenceId)
      ) {
        addIssue(
          context,
          ["occurrenceMappings"],
          "Occurrence supersession history must be bidirectional.",
        );
      }
    }
    for (const supersedingOccurrenceId of occurrence.supersededBy) {
      ...
      if (!superseding) {
        ...
        !superseding.supersedes.includes(occurrence.occurrenceId)
      ) {
        addIssue(
          context,
          ["occurrenceMappings"],
  ...
  for (const binding of artifact.occurrenceVariableBindings) {
    if (bindingOccurrenceIds.has(binding.occurrenceId)) {
      addIssue(
        context,
        ["occurrenceVariableBindings"],
        "An occurrence may have only one current binding decision.",
      );
    }
    ...
    if (!occurrence) {
      addIssue(
        context,
        ["occurrenceVariableBindings"],
        "Occurrence binding references an unknown occurrence.",
      );
    }
    ...
    if (!representedVariable) {
      addIssue(
        context,
        ["occurrenceVariableBindings"],
        "Occurrence binding references an unknown represented variable.",
      );
      continue;
    }
    ...
    if (
      ...
      binding.conceptualVariableId !==
        representedVariable.conceptualVariableId ||
      binding.conceptId !== conceptualVariable?.conceptId
    ) {
      addIssue(
        context,
        ["occurrenceVariableBindings"],
        "Occurrence binding references must match the represented-variable definition.",
      );
    }
    if (
      (representedVariable.valueScheme &&
        (!binding.valueScheme ||
          !sameScheme(representedVariable.valueScheme, binding.valueScheme))) ||
      (!representedVariable.valueScheme && binding.valueScheme)
    ) {
      addIssue(
        context,
        ["occurrenceVariableBindings"],
        "Occurrence binding value-scheme reference must match the represented-variable definition.",
      );
    }
    if (
      ...
      addIssue(
        context,
        ["occurrenceVariableBindings"],
        "Existing approved occurrence bindings must match the approved occurrence record.",
      );
    }
    if (
      occurrence?.reviewStatus === "approved" &&
      ...
      addIssue(
        context,
        ["occurrenceVariableBindings"],
        "Occurrence binding cannot replace an explicit approved occurrence binding.",
      );
    }
  }
  ...
  for (const decision of artifact.reviewDecisions) {
    if (!reviewableEntityIds.has(decision.entityId)) {
      addIssue(
        context,
        ["reviewDecisions"],
        "Review decision references an unknown ontology entity.",
      );
    }
  }
  ...
  for (const record of artifact.supersessionHistory) {
    ...
    if (!superseded || !superseding) {
      addIssue(
        context,
        ["supersessionHistory"],
        "Supersession history must reference known occurrences.",
      );
    }
    if (record.supersededOccurrenceId === record.supersedingOccurrenceId) {
      addIssue(
        context,
        ["supersessionHistory"],
        "Supersession history cannot be self-referential.",
      );
    }
    if (supersessionPairs.has(pair)) {
      addIssue(
        context,
        ["supersessionHistory"],
        "Supersession history cannot duplicate an occurrence pair.",
      );
    }
    ...
    if (
      ...
      addIssue(
        context,
        ["supersessionHistory"],
        "Occurrence supersession must cross workbook editions.",
      );
    }
    if (
      superseded &&
  ...
  if (
    ...
    addIssue(
      context,
      ["supersessionHistory"],
      "Occurrence supersession history must be acyclic.",
    );
  }
  for (const occurrence of artifact.occurrenceMappings.filter(
    isDurableOccurrence,
  )) {
    for (const supersededOccurrenceId of occurrence.supersedes) {
      ...
      if (!supersessionPairs.has(pair)) {
        addIssue(
          context,
          ["supersessionHistory"],
          "Durable occurrence supersession requires a history record.",
        );
      }
    }
  }
  ...
  for (const id of ids) {
    if (uniqueIds.has(id)) {
      addIssue(
        context,
        [],
        "Publication ontology entity IDs must be globally unique.",
      );
      return;
    }
    uniqueIds.add(id);
...
function localizedLabelKey(
  label: z.infer<typeof LocalizedLabelSchema>,
): string {
...
function schemeKeyOf(
  reference: z.infer<typeof ValueSchemeReferenceSchema>,
): string {
