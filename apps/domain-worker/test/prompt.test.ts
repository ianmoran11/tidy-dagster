...
import { loadPromptExamples } from "../src/prompt/examples.js";
import { schemaSkeletonExample } from "../src/prompt/promptVariants.js";
import { resolveSheetOntology } from "../src/prompt/ontology/overrides.js";
import type {
  OntologyDetection,
  SheetOntology,
} from "../src/prompt/ontology/types.js";
import { validateRecipe } from "../src/recipe/schema.js";
import type { GenerateRecipeRequest } from "../src/prompt/types.js";
import type { SheetSummary } from "../src/summary/types.js";
...
describe("ontology prompt construction", () => {
  it("serializes resolved edited ontology and broad-alias guidance", () => {
    ...
    expect(hint.joinCandidates).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ name: "fiscal_year" }),
      ]),
    );
    ...
    expect(prompt).toContain(
      "Prefer specific confirmed or edited ontology dimensions",
    );
  ...
  it("leaves prompt behavior unchanged when ontology is absent", () => {
    ...
    expect(prompt).not.toContain(
      "Use ontology_hints as deterministic JS-derived evidence",
    );
  });
});
...
describe("prompt variants", () => {
  ...
  it("keeps output shape specific to single vs multi sheet requests", () => {
    ...
    expect(sectionText(single, "output_shape")).toContain(
      "Return one RecipeV01 object.",
    );
    expect(sectionText(single, "output_shape")).not.toContain(
      '"recipes" array',
    );
  ...
  it("applies rule tiers and schema skeleton examples behind variant knobs", () => {
    ...
    expect(sectionText(skeleton, "examples")).toContain(
  ...
  it("applies table context compression and compact summaries only behind variant knobs", () => {
    ...
    expect(sectionText(baseline, "sheet_context")).toContain(
      "table_html_expanded",
    );
    expect(sectionText(compressed, "sheet_context")).toContain(
      "rows 2–4 blank",
    );
    expect(sectionText(compressed, "sheet_context")).toContain(
      "compact_summary_note",
    );
    expect(sectionText(compressed, "sheet_context")).toContain(
      "contextCells_omitted_count",
    );
    expect(sectionText(compressed, "sheet_context")).toContain(
      "header_list_omitted_count",
    );
    expect(sectionText(compressed, "sheet_context")).not.toContain(
      '"sizeChars"',
    );
    expect(sectionText(compressed, "sheet_context")).not.toContain(
      "table_html_expanded",
...
describe("prompt section instrumentation", () => {
  ...
  it("guards the default static prefix budget and section ids", () => {
    ...
    expect(DEFAULT_STATIC_PREFIX_CHAR_BUDGET).toBe(
      Math.ceil(staticChars * 1.1),
    );
  });
  ...
  it("reports section ids and accounts for cached prompt separators", () => {
    ...
    const cachedBlockJoinChars =
      staticSections.length > 0 && dynamicSections.length > 0 ? 2 : 0;
