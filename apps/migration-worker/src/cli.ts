...
function usage(): string {
  return "Usage: tidy-migration-worker --request FILE --input-root DIR --output-root DIR";
}

function cliFailure(code: string, message: string): MigrationWorkerResult {
  return {
...
function messageOf(error: unknown): string {
  return error instanceof Error
    ? error.message
    : "Migration worker CLI failed.";
}

function emit(result: MigrationWorkerResult): void {
  process.stdout.write(serializeMigrationWorkerResult(result));
}
