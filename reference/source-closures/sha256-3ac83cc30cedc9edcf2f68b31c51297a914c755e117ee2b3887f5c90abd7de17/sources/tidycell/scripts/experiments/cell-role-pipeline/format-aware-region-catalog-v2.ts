import { expandRange, formatRange, parseCell, parseRange } from "../../../src/lib/address";
import type { CellStyleSummary } from "../../../src/lib/workbook/types";
import type { CompactSemanticContext } from "./compact-context";
import {
...
export function buildFormatAwareSemanticRegionCatalog(
  ...
  return {
    ...
    omittedCandidateCount:
      base.omittedCandidateCount + Math.max(0, described.length - retained.length),
...
function buildFormatRuns(
  ...
  for (let row = 1; row <= context.dimensions.rows; row += 1) {
    ...
    for (let column = 1; column <= context.dimensions.columns + 1; column += 1) {
