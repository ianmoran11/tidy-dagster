...
import {
  SEMANTIC_TABLE_MAP_VERSION,
  type SemanticTableMapV1,
} from "./semantic-map-v1";
import { buildSharedContextSection } from "./prompts";
...
/**
 * A deliberately compact correction prompt: it does not repeat the full
 * workbook context or hundreds of irrelevant candidates.
 */
export function buildSemanticMapV13CorrectionPrompt({
  ...
  context: Pick<CompactContextSnapshot, "schemaVersion" | "digest">;
...
function semanticMapExample(): string {
  return JSON.stringify(
    {
      version: SEMANTIC_TABLE_MAP_VERSION,
      table: {
