...
export const CANONICAL_VALUE_MAPPER_ID = "deterministic_canonical_value_mapper" as const;
...
export type CanonicalValueMappingRequest = {
  ...
  validAt?: string;
  provenance: {
    sourceType: "publisher" | "external_standard" | "import" | "rule" | "user" | "migration";
    sourceId: string;
    recordedAt: string;
    evidence: string[];
    sourceUrl?: string;
...
export function mapCanonicalValue(
  ...
  const rejected = (reason: string): CanonicalValueMappingOutcome => ({
    mapping: abstainedMapping(request, normalized.value, [...baseEvidence, reason]),
    candidates: [],
    collision: false,
  });

  if (!request.rawValue.normalize("NFKC").trim()) return rejected("mapping.empty_raw_value");
  if (request.rawLanguage && !isLanguageTag(request.rawLanguage)) return rejected("mapping.invalid_raw_language");
  if (request.validAt && !isIsoDate(request.validAt)) return rejected("mapping.invalid_effective_date");
  if (request.publicationId !== context.profileId) return rejected("mapping.profile_mismatch");
  const variable = context.representedVariables.find((candidate) => candidate.id === request.representedVariableId);
  if (!variable) return rejected("mapping.unknown_represented_variable");
  if (!variable.valueScheme || !sameScheme(variable.valueScheme, request.valueScheme)) {
  ...
  const values = context.canonicalValues.filter(
    (value) => sameScheme(value.valueScheme, request.valueScheme) && isValidFor(value, request.validAt),
  );
  if (!values.length) return rejected("mapping.no_valid_canonical_values_for_scheme_version");

  const dictionaries = language
    ? context.approvedDictionaries.filter((dictionary) =>
  ...
  const publisherMatches = uniqueCandidates(
    dictionaries.flatMap((dictionary) =>
      applicableDictionaryEntries(dictionary, request.validAt).flatMap((entry) =>
        (entry.publisherCodes ?? [])
          .filter((code) => publisherCodes.includes(exactCanonicalValueText(code)))
          .flatMap(() => candidateFor(values, entry.canonicalValueId, "exact", "publisher_code", `publisher_code.${dictionary.id}@${dictionary.version}`)),
      ),
    ),
  );
  const publisherOutcome = resolvedOutcome(request, normalized.value, publisherMatches, baseEvidence);
  if (publisherOutcome) return publisherOutcome;

  const canonicalIdentifierMatches = uniqueCandidates([
    ...values.flatMap((value) => rawExact === value.id ? [candidate(value, "exact", "approved_dictionary", "canonical_id.exact")] : []),
    ...values.flatMap((value) => rawExact === exactCanonicalValueText(value.code) ? [candidate(value, "exact", "approved_dictionary", "canonical_code.exact")] : []),
  ]);
  const canonicalIdentifierOutcome = resolvedOutcome(request, normalized.value, canonicalIdentifierMatches, baseEvidence);
  if (canonicalIdentifierOutcome) return canonicalIdentifierOutcome;

  const directLabels = uniqueCandidates([
    ...values.flatMap((value) => labelsFor(value, language).flatMap((label) =>
      rawExact === exactCanonicalValueText(label.value)
        ? [candidate(value, label.kind === "preferred" ? "exact" : "alias", "approved_dictionary", `canonical_label.${label.kind}`)]
        : [],
    )),
    ...dictionaries.flatMap((dictionary) => applicableDictionaryEntries(dictionary, request.validAt).flatMap((entry) =>
      (entry.aliases ?? []).flatMap((alias) => rawExact === exactCanonicalValueText(alias)
        ? candidateFor(values, entry.canonicalValueId, "alias", "approved_dictionary", `dictionary.${dictionary.id}@${dictionary.version}`)
        : [],
      ),
    )),
  ]);
  const directLabelOutcome = resolvedOutcome(request, normalized.value, directLabels, baseEvidence);
  ...
  const normalizedMatches = uniqueCandidates([
    ...values.flatMap((value) => labelsFor(value, language).flatMap((label) =>
      normalizeCanonicalValue(label.value).value === normalized.value
        ? [candidate(value, "alias", "rule", `normalized_canonical_label.${label.kind}`)]
        : [],
    )),
    ...dictionaries.flatMap((dictionary) => applicableDictionaryEntries(dictionary, request.validAt).flatMap((entry) =>
      (entry.aliases ?? []).flatMap((alias) => normalizeCanonicalValue(alias).value === normalized.value
        ? candidateFor(values, entry.canonicalValueId, "alias", "rule", `normalized_dictionary.${dictionary.id}@${dictionary.version}`)
        : [],
      ),
    )),
  ]);
  const normalizedOutcome = resolvedOutcome(request, normalized.value, normalizedMatches, baseEvidence);
...
function resolvedOutcome(
  ...
  const ids = [...new Set(candidates.map((candidate) => candidate.value.id))].sort();
  const protectedSourceValues = [
    normalizedValue,
    ...(request.publisherCode ? [normalizeCanonicalValue(request.publisherCode).value] : []),
  ];
  const unsafeCandidates = candidates
    .map((candidate) => ({ candidate, reason: protectedHardNegativeReason(protectedSourceValues, candidate.value) }))
    .filter((entry): entry is { candidate: Candidate; reason: string } => Boolean(entry.reason));
  if (unsafeCandidates.length) {
    return {
      mapping: abstainedMapping(request, normalizedValue, [
        ...baseEvidence,
        "mapping.protected_hard_negative",
        ...unsafeCandidates.map(({ candidate, reason }) => `${reason}.${candidate.value.id}`).sort(),
  ...
  if (ids.length !== 1) {
    return {
      mapping: abstainedMapping(request, normalizedValue, [...baseEvidence, "mapping.normalization_collision", ...ids.map((id) => `candidate.${id}`)]),
      candidates: ids,
      collision: true,
    };
  }
  ...
  return {
    mapping: {
      ...
      ...(request.publisherCode && publisherCodeKey(request.publisherCode) ? { publisherCode: request.publisherCode } : {}),
      ...(request.rawLanguage && isLanguageTag(request.rawLanguage) ? { rawLanguage: request.rawLanguage } : {}),
      normalizedValue,
      ...(request.validAt && isIsoDate(request.validAt) ? { effectiveAt: request.validAt } : {}),
      normalizationMethod: selected.method,
      normalizationMethodVersion: selected.method === "rule"
        ? CANONICAL_VALUE_NORMALIZATION_VERSION
        : CANONICAL_VALUE_MAPPER_VERSION,
      canonicalValueId: selected.value.id,
      relation: selected.relation,
      confidence: selected.method === "publisher_code" ? 1 : selected.method === "approved_dictionary" ? 0.999 : 0.998,
...
function labelsFor(value: MappingCanonicalValue, language: string | undefined): Array<{ kind: "preferred" | "alternative" | "hidden"; value: string }> {
  if (!language) return [];
  return [
    ...value.preferredLabels.map((label) => ({ ...label, kind: "preferred" as const })),
    ...(value.alternativeLabels ?? []).map((label) => ({ ...label, kind: "alternative" as const })),
    ...(value.hiddenLabels ?? []).map((label) => ({ ...label, kind: "hidden" as const })),
  ].filter((label) => sameLanguage(label.language, language));
}

function candidate(value: MappingCanonicalValue, relation: Candidate["relation"], method: Candidate["method"], evidence: string): Candidate {
  return { value, relation, method, evidence };
}
function candidateFor(values: MappingCanonicalValue[], id: string, relation: Candidate["relation"], method: Candidate["method"], evidence: string): Candidate[] {
  return values.filter((value) => value.id === id).map((value) => candidate(value, relation, method, evidence));
}
...
function compareCandidates(left: Candidate, right: Candidate): number {
  const relationRank = (relation: Candidate["relation"]) => relation === "exact" ? 0 : 1;
...
function sameScheme(left: ValueSchemeReference, right: ValueSchemeReference): boolean {
  return left.valueSchemeId === right.valueSchemeId && left.valueSchemeVersion === right.valueSchemeVersion;
}
...
function isValidFor(value: MappingCanonicalValue, validAt: string | undefined): boolean {
  ...
  if (bounded) return Boolean(validAt) && isValidityWindowApplicable(value, validAt);
  return true;
}
...
function isValidityWindowApplicable(
  ...
  return (!value.validFrom || value.validFrom <= validAt) &&
...
function applicableDictionaryEntries(
  ...
  return dictionary.entries.filter((entry) => isValidityWindowApplicable(entry, validAt));
}
...
function protectedHardNegativeReason(
  ...
  if (
    normalizedSourceValues.some((value) => new Set(["men", "boys", "persons"]).has(value)) &&
    target.demographicRole !== "other"
  ) {
    return "mapping.people_label_requires_explicit_non_male_target";
  }
  if (
    normalizedSourceValues.some((value) => new Set(["total", "totals"]).has(value)) &&
...
function mappingIdFor(request: CanonicalValueMappingRequest): string {
  const text = [request.publicationId, request.representedVariableId, request.valueScheme.valueSchemeId, request.valueScheme.valueSchemeVersion, request.rawLanguage ?? "", request.validAt ?? "", publisherCodeKey(request.publisherCode), request.rawValue].join("\u001f");
  let hash = 2166136261;
...
export type CanonicalValueMappingEvaluationCase = {
  ...
  expectedGoldRelation?: "exact" | "alias" | "close" | "broader" | "narrower" | "unmapped";
};
...
export type CanonicalValueMappingReport = {
  reportVersion: "1.0";
  mapper: { id: typeof CANONICAL_VALUE_MAPPER_ID; version: typeof CANONICAL_VALUE_MAPPER_VERSION };
  counts: { automatic: number; abstained: number; collisions: number };
  evidence: CanonicalValueMappingEvidence;
  heldOutAutomaticMergePrecision: { correct: number; total: number; value: number | null };
  promotion: {
  ...
  mappings: Array<{ id: string; publicationId?: string; editionId?: string; split: CanonicalValueMappingEvaluationCase["split"]; publisherCode?: string; canonicalValueId?: string; relation: RawValueMapping["relation"]; expectedRelation: RawValueMapping["relation"]; expectedGoldRelation: "exact" | "alias" | "close" | "broader" | "narrower" | "unmapped"; expectedCanonicalValueId?: string; reviewStatus: RawValueMapping["reviewStatus"]; confidence: number; correct: boolean; recoveredGoldMapping: boolean; collision: boolean }>;
};
...
export function reportCanonicalValueMappings(
  ...
  evidence: CanonicalValueMappingEvidence = {
    ...
    hardNegativeCount: cases.filter((entry) => entry.expectedRelation === "unmapped").length,
    sufficient: false,
    missing: ["PRD 002 publication/edition-held-out evidence floor was not supplied"],
  },
): CanonicalValueMappingReport {
  const results = cases.map((entry) => ({ entry, outcome: mapCanonicalValue(entry.context ?? context, entry.request) }));
  const heldOut = results.filter(({ entry, outcome }) => entry.split !== "tuning" && outcome.mapping.reviewStatus === "automatic");
  const correct = heldOut.filter(({ entry, outcome }) =>
    outcome.mapping.canonicalValueId === entry.expectedCanonicalValueId && evaluationRelationsCompatible(entry.expectedGoldRelation ?? entry.expectedRelation, outcome.mapping.relation),
  ).length;
  const precision = heldOut.length ? correct / heldOut.length : null;
  const lower = heldOut.length ? oneSidedExactBinomialLowerBound(correct, heldOut.length) : null;
  const reasons = evidence.sufficient
    ? [
      ...(precision === null ? ["No automatic held-out mappings were evaluated."] : []),
      ...(precision !== null && precision < 0.995 ? ["Observed automatic-merge precision is below 99.5%."] : []),
      ...(lower !== null && lower < 0.995 ? ["One-sided 95% exact-binomial lower bound is below 99.5%."] : []),
    ]
    : [...evidence.missing].sort();
  const status = !evidence.sufficient
    ? "insufficient_evidence"
    : reasons.length ? "review_only" : "eligible_for_promotion";
  return {
    reportVersion: "1.0",
    mapper: { id: CANONICAL_VALUE_MAPPER_ID, version: CANONICAL_VALUE_MAPPER_VERSION },
    counts: {
      automatic: results.filter(({ outcome }) => outcome.mapping.reviewStatus === "automatic").length,
      abstained: results.filter(({ outcome }) => outcome.mapping.reviewStatus === "abstained").length,
      collisions: results.filter(({ outcome }) => outcome.collision).length,
    },
    evidence: { ...evidence, missing: [...evidence.missing].sort() },
    heldOutAutomaticMergePrecision: { correct, total: heldOut.length, value: precision },
    promotion: { status, threshold: 0.995, oneSided95LowerBound: lower, reasons: reasons.length ? reasons : ["PRD 002 promotion criteria met."] },
    mappings: results.map(({ entry, outcome }) => ({
      id: entry.id,
      ...
      expectedGoldRelation: entry.expectedGoldRelation ?? entry.expectedRelation,
      ...
      correct: entry.expectedRelation === "unmapped"
        ? outcome.mapping.reviewStatus === "abstained" && outcome.mapping.relation === "unmapped"
        : outcome.mapping.canonicalValueId === entry.expectedCanonicalValueId && evaluationRelationsCompatible(entry.expectedGoldRelation ?? entry.expectedRelation, outcome.mapping.relation),
      recoveredGoldMapping: outcome.mapping.reviewStatus === "automatic" &&
        outcome.mapping.canonicalValueId === entry.expectedCanonicalValueId &&
        evaluationRelationsCompatible(entry.expectedGoldRelation ?? entry.expectedRelation, outcome.mapping.relation),
...
function evaluationRelationsCompatible(
  ...
  return (expected === "exact" || expected === "alias") && (observed === "exact" || observed === "alias");
}

export function stableCanonicalValueMappingJson(report: CanonicalValueMappingReport): string {
  return `${JSON.stringify(sortObjectKeys(report), null, 2)}\n`;
}

export function canonicalValueMappingMarkdown(report: CanonicalValueMappingReport): string {
...
export function oneSidedExactBinomialLowerBound(successes: number, trials: number, alpha = 0.05): number {
  if (!Number.isInteger(successes) || !Number.isInteger(trials) || successes < 0 || trials < successes || !(alpha > 0 && alpha < 1)) {
  ...
  for (let iteration = 0; iteration < 100; iteration += 1) {
    ...
    if (regularizedBeta(midpoint, successes, trials - successes + 1) > alpha) upper = midpoint;
    else lower = midpoint;
  }
...
function logBeta(a: number, b: number): number { return logGamma(a) + logGamma(b) - logGamma(a + b); }
function logGamma(value: number): number {
  const coefficients = [Number("76.18009172947146"), Number("-86.50532032941677"), 24.01409824083091, -1.231739572450155, 0.001208650973866179, -0.000005395239384953];
  let x = value - 1;
  const tmp = x + 5.5 - (x + 0.5) * Math.log(x + 5.5);
  let series = Number("1.000000000190015");
  for (const coefficient of coefficients) { x += 1; series += coefficient / x; }
...
function betaFraction(x: number, a: number, b: number): number {
  ...
  for (let m = 1; m <= 200; m += 1) {
    ...
    d = 1 + aa * d; d = Math.abs(d) < minimum ? minimum : d; c = 1 + aa / c; c = Math.abs(c) < minimum ? minimum : c; d = 1 / d; h *= d * c;
    aa = -(a + m) * (a + b + m) * x / ((a + m2) * (a + m2 + 1));
    d = 1 + aa * d; d = Math.abs(d) < minimum ? minimum : d; c = 1 + aa / c; c = Math.abs(c) < minimum ? minimum : c; d = 1 / d;
    const delta = d * c;
    h *= delta;
...
function sortObjectKeys(value: unknown): unknown {
  ...
  if (value && typeof value === "object") return Object.fromEntries(Object.entries(value as Record<string, unknown>).sort(([a], [b]) => a.localeCompare(b)).map(([key, child]) => [key, sortObjectKeys(child)]));
  return value;
}
