...
export function buildCalendarRollupCandidates(input: {
  ...
  if (hierarchy.type !== "calendar" || hierarchy.periodSchemeKind !== "calendar" || !hierarchy.periodScheme || !sameScheme(hierarchy.periodScheme, periodScheme)) {
    return [];
  }
  const levels = new Set(hierarchy.levels.map((level) => level.id));
  if (!levels.has("year") || !levels.has("quarter") || !levels.has("month")) return [];
  ...
  const entries = [...valueByCode.entries()].sort(([left], [right]) => left.localeCompare(right));
  for (const [monthCode, month] of entries) {
    ...
    if (quarterValue) candidates.push(edge(hierarchy, month.id, quarterValue.id, "month", "quarter"));
  }
  for (const [quarterCode, quarter] of entries) {
    const match = /^(\d{4})-Q[1-4]$/.exec(quarterCode);
    if (!match) continue;
    const yearValue = valueByCode.get(match[1]);
    if (yearValue) candidates.push(edge(hierarchy, quarter.id, yearValue.id, "quarter", "year"));
  }
  ...
  return candidates
    ...
    .sort((left, right) => [left.sourceId, left.targetId].join("\u001f").localeCompare([right.sourceId, right.targetId].join("\u001f")));
}
