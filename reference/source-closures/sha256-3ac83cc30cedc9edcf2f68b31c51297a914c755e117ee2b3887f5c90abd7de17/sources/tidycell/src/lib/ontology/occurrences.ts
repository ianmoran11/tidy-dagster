import { occurrenceIdFor, type PublicationEntityId } from "./publicationIds";
import type { OntologyArtifact } from "./overrides";
import type { OntologyDetection } from "./types";
...
export function occurrenceMatchesDetectionSource(
  ...
  return occurrence.publicationId === context.publicationId
...
export function createOccurrenceStructuralSignature(
  ...
  const evidence: OccurrenceStructuralEvidence = {
    tableContext: {
      ...
      neighbouringHeaders: normalizeTokens(input.tableContext.neighbouringHeaders),
    },
    ...
    ...(input.unitScale ? { unitScale: normalizeOptional(input.unitScale) } : {}),
...
export function createDurableOccurrence(
  ...
  const structuralSignature = createOccurrenceStructuralSignature(input.structuralEvidence);
  const rawDetectionIds = sortedUniqueExact(input.rawDetectionIds, "raw detection IDs");
  ...
  if (
    ...
    rawDetectionIds.some((detectionId) => normalizeOptional(detectionId) === durableOccurrenceKey)
  ) {
    throw new Error("Raw detection IDs cannot be repurposed as durable occurrence keys.");
  }
  const occurrenceId =
    input.occurrenceId ??
    occurrenceIdFor(input.publicationId, input.workbookEditionId, durableOccurrenceKey);
  if (input.reviewStatus === "approved" && !input.representedVariableId) {
    throw new Error("Approved occurrences require a represented-variable binding.");
  }
  if (input.reviewStatus !== "approved" && input.representedVariableId) {
    throw new Error("Only approved occurrences may carry a represented-variable binding.");
  }
  if (rawDetectionIds.some((id) => id === occurrenceId)) {
    throw new Error("Raw detection IDs cannot be used as durable occurrence IDs.");
  }

  return {
    occurrenceId,
    publicationId: input.publicationId,
    workbookEditionId: normalizeRequired(input.workbookEditionId, "workbook edition ID"),
    ...
    supersededBy: sortedUniqueEntityIds(input.supersededBy ?? [], "supersededBy"),
  };
}
...
export function reconcileOccurrences({
  ...
  if (minimumScore < 0 || minimumScore > 1 || minimumMargin < 0 || minimumMargin > 1) {
    throw new Error("Reconciliation score and margin thresholds must be between zero and one.");
  }

  const previous = new Map(
    [...priorOccurrences]
      .sort(compareOccurrence)
      .map((occurrence) => [occurrence.occurrenceId, cloneOccurrence(occurrence)]),
  );
  ...
  for (const incoming of [...output.values()].sort(compareOccurrence)) {
    ...
    const eligible = candidates.filter((candidate) => candidate.blockingReasons.length === 0);
    const best = eligible[0];
    const runnerUp = eligible[1];
    const reviewReasons: ReconciliationReviewReason[] = [];

    if (best) {
      if (runnerUp && almostEqual(best.score, runnerUp.score)) reviewReasons.push("tied_candidates");
      else if (runnerUp && best.score - runnerUp.score < minimumMargin) reviewReasons.push("low_margin");
    ...
    const candidateReviewReasons = incompatibleReasons.filter(
      (reason) =>
        !["different_publication", "incompatible_table_context"].includes(reason),
    );
    ...
    if (!best) {
      ...
      if (isAmbiguous) {
        output.set(incoming.occurrenceId, abstainedUnlessAlreadyApproved(incoming));
      }
      entries.push(entry);
      continue;
    }
    ...
    if (!chosenPrior) throw new Error("Reconciliation candidate disappeared during matching.");
    ...
    if (hasConflictingApprovedCurrentBinding) reviewReasons.push("conflicting_current_binding");
    const safeMatch = reviewReasons.length === 0 && !usedPriorIds.has(chosenPrior.occurrenceId);
    if (!safeMatch) {
      const reasons = sortedUnique(
        [
          ...reviewReasons,
          ...(usedPriorIds.has(chosenPrior.occurrenceId) ? ["tied_candidates"] : []),
        ],
        "review reasons",
      ) as ReconciliationReviewReason[];
      output.set(incoming.occurrenceId, abstainedUnlessAlreadyApproved(incoming));
    ...
    const updatedIncoming: DurableVariableOccurrence = {
      ...incoming,
      reviewStatus: transferredApprovedBinding ? "approved" : incoming.reviewStatus,
    ...
    const locationUnchanged = sameLocation(incoming.sourceLocation, chosenPrior.sourceLocation);
    const entry: OccurrenceReconciliationEntry = {
      occurrenceId: incoming.occurrenceId,
      status: locationUnchanged ? "retained" : "moved",
      matchedOccurrenceId: chosenPrior.occurrenceId,
      transferredApprovedBinding,
      reviewRequired: !transferredApprovedBinding && !preserveCurrentApprovedBinding,
      reviewReasons:
...
export function scoreOccurrenceCandidate(
  ...
  if (incoming.publicationId !== prior.publicationId) blockingReasons.push("different_publication");
  if (!sameTableContext(incoming.structuralEvidence, prior.structuralEvidence)) {
  ...
  if (classificationSimilarity === 0) blockingReasons.push("conflicting_classification_version");

  const components: ReconciliationScoreComponent[] = [
    component("table_context", 0.2, sameTableContext(incoming.structuralEvidence, prior.structuralEvidence) ? 1 : 0),
    component(
      "neighbouring_headers",
      0.15,
      jaccardSimilarity(
    ...
    component(
      "orientation",
      0.1,
      incoming.structuralEvidence.orientation === prior.structuralEvidence.orientation ? 1 : 0,
    ),
    ...
    component(
      ...
      incoming.structuralEvidence.priorRepresentedVariableId &&
        ...
        incoming.structuralEvidence.priorRepresentedVariableId === prior.representedVariableId
        ? 1
        : incoming.structuralEvidence.priorRepresentedVariableId || prior.representedVariableId
          ? 0.5
          : 1,
  ...
  return {
    ...
    score: round(components.reduce((sum, entry) => sum + entry.contribution, 0)),
    components,
    blockingReasons: sortedUnique(blockingReasons, "blocking reasons") as ReconciliationReviewReason[],
  };
}
...
export function occurrenceCandidatesFromSheetArtifact({
  ...
  const prepared = artifact.resolvedDetections.map((detection) => {
    const structuralEvidence: OccurrenceStructuralEvidence = {
      tableContext: {
        ...
        neighbouringHeaders: neighbouringHeadersByDetectionId[detection.id] ?? [],
      },
    ...
    return {
      detection,
      structuralEvidence,
      structuralSignature: createOccurrenceStructuralSignature(structuralEvidence),
    };
  });
  ...
  return prepared
    .sort((left, right) =>
      left.structuralSignature.signature.localeCompare(right.structuralSignature.signature) ||
    ...
    .map(({ detection, structuralEvidence, structuralSignature }) => {
      ...
      if (!suppliedKey && (signatureCounts.get(structuralSignature.signature) ?? 0) > 1) {
        throw new Error(
...
function abstainedUnlessAlreadyApproved(
  ...
  return { ...occurrence, reviewStatus: "abstained", representedVariableId: undefined };
}
...
function buildReport(
  ...
  const sortEntries = (status: OccurrenceReconciliationEntry["status"]) =>
    entries.filter((entry) => entry.status === status).sort((left, right) => left.occurrenceId.localeCompare(right.occurrenceId));
  return {
    version: "1.0",
...
function component(
  ...
  return { name, weight, similarity: round(similarity), contribution: round(weight * similarity) };
}

function compareCandidate(left: ReconciliationCandidate, right: ReconciliationCandidate): number {
  return right.score - left.score || left.occurrenceId.localeCompare(right.occurrenceId);
}

function compareOccurrence(left: DurableVariableOccurrence, right: DurableVariableOccurrence): number {
...
function sameTableContext(
  ...
  return (
    normalizeOptional(left.tableContext.id) === normalizeOptional(right.tableContext.id) &&
    normalizeOptional(left.tableContext.kind) === normalizeOptional(right.tableContext.kind)
  );
}
...
function sameLocation(left: OccurrenceSourceLocation, right: OccurrenceSourceLocation): boolean {
  return (
    left.sheetName === right.sheetName &&
    left.range === right.range &&
    JSON.stringify([...left.addresses].sort()) === JSON.stringify([...right.addresses].sort())
  );
}

function cloneOccurrence(occurrence: DurableVariableOccurrence): DurableVariableOccurrence {
  return {
    ...occurrence,
    sourceLocation: normalizeSourceLocation(occurrence.sourceLocation),
    rawDetectionIds: [...occurrence.rawDetectionIds],
    structuralEvidence: createOccurrenceStructuralSignature(occurrence.structuralEvidence).evidence,
...
function cloneProvenance(provenance: OccurrenceProvenance): OccurrenceProvenance {
  return { ...provenance, evidence: [...provenance.evidence] };
}

function normalizeSourceLocation(sourceLocation: OccurrenceSourceLocation): OccurrenceSourceLocation {
  return {
...
function assertUniqueOccurrenceIds(
  ...
  if (new Set(ids).size !== ids.length) {
    throw new Error(`Duplicate ${description} occurrence IDs are not reconcilable.`);
  }
}

function sortedUnique(values: readonly string[], description: string): string[] {
  const normalized = values.map((value) => normalizeRequired(value, description));
...
function sortedUniqueExact(values: readonly string[], description: string): string[] {
...
function normalizeTokens(values: readonly string[]): string[] {
  return [...new Set(values.map((value) => normalizeOptional(value)).filter(Boolean))].sort();
}
