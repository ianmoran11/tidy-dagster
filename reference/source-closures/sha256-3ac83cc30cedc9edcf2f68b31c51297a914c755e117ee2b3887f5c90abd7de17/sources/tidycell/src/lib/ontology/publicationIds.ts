...
const ID_PATTERN = /^(publisher|publication|concept|variable|value_scheme|value|hierarchy|occurrence)_[a-z0-9]{8,32}$/;
...
export function stablePublicationEntityId(
  ...
  if (stableParts.length === 0 || stableParts.some((part) => !part.trim())) {
    throw new Error("Stable publication entity IDs require at least one non-empty durable part.");
  }

  return `${prefix}_${fnv1a32(stableParts.map(normalizeStablePart).join("\u001f"))}`;
}

export function publisherIdFor(authority: string, registryId: string): PublicationEntityId {
...
export function publicationIdFor(
  ...
  return stablePublicationEntityId("publication", [publisherId, durablePublicationId]);
}
...
export function externalConceptSchemeIdFor(
  ...
  const conceptId = stablePublicationEntityId("concept", [authority, durableSchemeId]);
