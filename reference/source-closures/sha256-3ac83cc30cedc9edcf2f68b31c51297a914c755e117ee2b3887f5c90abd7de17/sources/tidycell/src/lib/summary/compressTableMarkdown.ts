import type { CandidateRegion } from "@/lib/summary/types";
...
export function compressTableMarkdown(
  ...
  if (options.cellCharCap !== undefined) {
    rows = rows.map((row) => ({
      ...row,
      cells: row.cells.map((cell) => capCellText(cell, options.cellCharCap ?? DEFAULT_CELL_CHAR_CAP)),
    })).map(materializeRow);
  }
...
function extractRowIndex(line: string): number | null {
  const rows = [...line.matchAll(/R(\d+)C\d+/gi)].map((match) => Number(match[1]));
...
function isBlankDataRow(row: MarkdownRow): boolean {
  return !row.isSeparator && row.cells.length > 1 && row.cells.every((cell) => cellSemanticText(cell) === "");
}
...
function collapseBlankRows(rows: MarkdownRow[]): MarkdownRow[] {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (count >= 3) {
      result.push(annotationRow(`… rows ${rows[index]?.rowIndex ?? index + 1}–${rows[end]?.rowIndex ?? end + 1} blank …`));
    } else {
...
function normalizedRow(row: MarkdownRow): string {
  return row.cells.map((cell) => cellSemanticText(cell).toLowerCase()).join("|");
}
...
function collapseRepeatedRows(rows: MarkdownRow[]): MarkdownRow[] {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    if (count >= 3) {
      result.push(row);
      result.push(annotationRow(`… rows ${rows[index]?.rowIndex ?? index + 1}–${rows[end]?.rowIndex ?? end + 1} repeat previous row ${count - 1} more times …`));
    } else {
      result.push(...rows.slice(index, end + 1));
    }
    index = end;
...
function collapseBlankColumns(rows: MarkdownRow[]): MarkdownRow[] {
  const tableRows = rows.filter((row) => row.cells.length > 1 && !row.isSeparator);
  const maxColumns = Math.max(0, ...tableRows.map((row) => row.cells.length));
  const blankColumns: boolean[] = [];

  for (let col = 0; col < maxColumns; col += 1) {
    blankColumns[col] = tableRows.length > 0 && tableRows.every((row) => cellSemanticText(row.cells[col] ?? "") === "");
  }
  ...
  return rows.map((row) => {
    ...
    for (let col = 0; col < row.cells.length; col += 1) {
      ...
      if (run) {
        ...
        col = run.end;
      } else if (!runs.some((candidate) => col > candidate.start && col <= candidate.end)) {
...
function capCellText(cell: string, maxChars: number): string {
  if (!Number.isFinite(maxChars) || maxChars <= 0 || cell.length <= maxChars) return cell;
...
function sampleRows(
  ...
  const sampling = typeof options.rowSampling === "object" ? options.rowSampling : {};
  const firstRows = sampling.firstRows ?? 12;
  const lastRows = sampling.lastRows ?? 8;
  const boundaryPadding = sampling.boundaryPadding ?? 2;
  const dataRows = rows.filter((row) => !row.isSeparator && row.cells.length > 1);
  ...
  const boundaries = (options.candidateRegions ?? []).flatMap((region) => rangeRows(region.range));
  for (const row of dataRows) {
    const rowIndex = row.rowIndex ?? 0;
    if (boundaries.some((boundary) => Math.abs(rowIndex - boundary) <= boundaryPadding)) {
      keep.add(row);
    }
  }
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    result.push(annotationRow(`… rows ${rows[index]?.rowIndex ?? index + 1}–${rows[end]?.rowIndex ?? end + 1} elided by sampling …`));
    index = end;
  }
  return result;
}
...
function rangeRows(range: string): number[] {
  ...
  if (!match) return [];
  return [Number(match[1]), Number(match[2] ?? match[1])].filter(Number.isFinite);
}
