...
function main(): void {
  ...
  if (process.platform !== "darwin" || !pathExists("/usr/bin/sandbox-exec"))
    throw new Error(
      "Historical compact-context reference requires macOS sandbox-exec",
    );
  ...
  const replay = mkdtempSync(
    path.join(replayParent, "compact-context-reference-"),
  );
  try {
    ...
    const rawOutput = path.join(replay, "source-compact-context-output.json");
    const runner = path.join(
      replay,
      "source-compact-context-reference-runner.ts",
    );
    cpSync(
      path.join(project, "scripts/source-compact-context-reference-runner.ts"),
      runner,
      {
        errorOnExist: true,
      },
    ...
    const profilePath = path.join(
      replay,
      "source-compact-context-reference.sb",
    );
    ...
    if (child.status !== 0) {
      ...
      throw new Error(
        `Historical compact-context reference failed (${child.status}):\n${diagnostic}`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Historical compact-context reference returned unexpected cases",
      );
    const cases = rawCases.map((entry) => {
      ...
      if (workbookRelativePath !== `fixtures/workbooks/${caseId}.xlsx`)
        throw new Error(
          "Historical compact-context case has an unexpected workbook path",
        );
      const sourceItem = itemByPath.get(workbookRelativePath);
      if (!sourceItem || sourceItem.role !== "fixture")
        throw new Error(
          "Historical compact-context workbook lacks copied custody",
        );
      if (!Array.isArray(entry.contexts) || entry.contexts.length < 1)
        throw new Error("Historical compact-context case has no contexts");
      const sheetNames = entry.contexts.map((context: unknown) => {
        if (!context || typeof context !== "object" || Array.isArray(context))
          throw new Error(
            "Historical compact-context snapshot is not an object",
          );
        const snapshot = context as Record<string, unknown>;
        if (typeof snapshot.serialized !== "string")
          throw new Error("Compact-context serialization is invalid");
        const parsed = parseObject(Buffer.from(snapshot.serialized));
        if (typeof parsed.sheet !== "string" || !parsed.sheet)
          throw new Error("Compact-context sheet is invalid");
        const digest = createHash("sha256")
          .update(Buffer.from(snapshot.serialized))
          .digest("hex");
        if (
          snapshot.digest !== digest ||
          snapshot.characters !== snapshot.serialized.length ||
          snapshot.bytes !== Buffer.byteLength(snapshot.serialized)
        )
          throw new Error("Compact-context snapshot identity is invalid");
        return parsed.sheet;
      });
      if (new Set(sheetNames).size !== sheetNames.length)
        throw new Error("Historical compact-context sheets are duplicated");
      const semanticCase = {
        caseId,
    ...
    if (before !== after)
      throw new Error(
        "Historical compact-context execution changed copied source bytes",
      );
    verifyBundle(bundle);

    const harnessDigest = domainDigest(
      "tidy.historical-source-compact-context-reference-harness/v1",
      {
        files: [
          "scripts/freeze-source-compact-context-reference.ts",
          "scripts/source-compact-context-reference-runner.ts",
          "contracts/reference-context/v1/reference.schema.json",
          "package.json",
          "package-lock.json",
          ".node-version",
    ...
    const semantic = {
      ...
      referenceKind: "compact-complete-semantic-context",
      contextContract: {
      ...
      limitations: [
        "This is a historical-source compact-context reference generated under implementing-agent self-review.",
    ...
    process.stdout.write(
      `${canonicalJson({
        ...
        contextCount: cases.reduce(
          (total, entry) => total + entry.contextCount,
          0,
        ),
        parityEstablished: false,
      })}\n`,
...
function parseArgs(argv: string[]): Args {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!flag?.startsWith("--") || !value || values.has(flag))
      throw new Error("Invalid historical compact-context reference arguments");
