...
const schemaVersion = "tidy.historical-source-region-catalog-reference/v1";
const caseVersion = "tidy.historical-source-region-catalog-reference-case/v1";
...
function main(): void {
  ...
  if (process.platform !== "darwin" || !pathExists("/usr/bin/sandbox-exec"))
    throw new Error(
      "Historical region-catalog reference requires macOS sandbox-exec",
    );
  ...
  const replay = mkdtempSync(
    path.join(replayParent, "region-catalog-reference-"),
  );
  try {
    ...
    const rawOutput = path.join(replay, "source-region-catalog-output.json");
    const runner = path.join(
      replay,
      "source-region-catalog-reference-runner.ts",
    );
    cpSync(
      path.join(project, "scripts/source-region-catalog-reference-runner.ts"),
      runner,
      {
        errorOnExist: true,
      },
    ...
    const profilePath = path.join(replay, "source-region-catalog-reference.sb");
    ...
    if (child.status !== 0) {
      ...
      throw new Error(
        `Historical region-catalog reference failed (${child.status}):\n${diagnostic}`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Historical region-catalog reference returned unexpected cases",
      );
    const cases = rawCases.map((entry) => {
      ...
      if (workbookRelativePath !== `fixtures/workbooks/${caseId}.xlsx`)
        throw new Error(
          "Historical region-catalog case has an unexpected workbook path",
        );
      const sourceItem = itemByPath.get(workbookRelativePath);
      if (!sourceItem || sourceItem.role !== "fixture")
        throw new Error(
          "Historical region-catalog workbook lacks copied custody",
        );
      if (!Array.isArray(entry.catalogs) || entry.catalogs.length < 1)
        throw new Error("Historical region-catalog case has no catalogs");
      const sheetNames = entry.catalogs.map((catalog: unknown) => {
        if (!catalog || typeof catalog !== "object" || Array.isArray(catalog))
          throw new Error("Historical region catalog is not an object");
        const value = catalog as Record<string, unknown>;
        if (
          value.version !== "semantic-region-catalog-v5-adjacent-year-aware" ||
          typeof value.sheet !== "string" ||
          !value.sheet ||
          !Array.isArray(value.candidates) ||
          value.candidates.length > 160
        )
          throw new Error("Historical region catalog contract is invalid");
        return value.sheet;
      });
      if (new Set(sheetNames).size !== sheetNames.length)
        throw new Error("Historical region-catalog sheets are duplicated");
      const semanticCase = {
        caseId,
        workbookRelativePath,
        workbookContentDigest: String(sourceItem.contentDigest),
        catalogCount: entry.catalogs.length,
    ...
    if (before !== after)
      throw new Error(
        "Historical region-catalog execution changed copied source bytes",
      );
    verifyBundle(bundle);

    const harnessDigest = domainDigest(
      "tidy.historical-source-region-catalog-reference-harness/v1",
      {
        files: [
          "scripts/freeze-source-region-catalog-reference.ts",
          "scripts/source-region-catalog-reference-runner.ts",
          "contracts/reference-region/v1/reference.schema.json",
          "package.json",
          "package-lock.json",
          ".node-version",
    ...
    const semantic = {
      ...
      referenceKind: "role-aware-semantic-region-catalog",
      catalogContract: {
      ...
      limitations: [
        "This is a historical-source role-aware region-catalog reference generated under implementing-agent self-review.",
    ...
    process.stdout.write(
      `${canonicalJson({
        ...
        catalogCount: cases.reduce(
          (total, entry) => total + entry.catalogCount,
          0,
        ),
        parityEstablished: false,
      })}\n`,
...
function parseArgs(argv: string[]): Args {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!flag?.startsWith("--") || !value || values.has(flag))
      throw new Error("Invalid historical region-catalog reference arguments");
