...
const formatModule = await import(
  pathToFileURL(
    path.join(
      root,
      "scripts/experiments/cell-role-pipeline/format-aware-region-catalog-v2.ts",
    ),
  ).href
);
const catalogModule = await import(
  pathToFileURL(
    path.join(
      root,
      "scripts/experiments/cell-role-pipeline/role-aware-region-catalog-v5.ts",
    ),
  ).href
);
...
for (const workbookName of workbookNames) {
  ...
  cases.push({
    ...
    catalogs: parsed.workbook.sheets.map((sheet: any) => {
      const context = contextModule.buildCompactSemanticContext(sheet);
      return catalogModule.buildRoleAwareSemanticRegionCatalog(context, {
        formattingFacts: formatModule.buildSemanticCellFormattingFacts(
          sheet.cells,
        ),
        cellDataFacts: catalogModule.buildSemanticCellDataFacts(sheet.cells),
      });
    }),
  });
}
