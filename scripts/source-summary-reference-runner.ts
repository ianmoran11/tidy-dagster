...
const summaryModule = await import(
  pathToFileURL(path.join(root, "src/lib/summary/buildSheetSummary.ts")).href
);
...
for (const workbookName of workbookNames) {
  ...
  cases.push({
    ...
    summaries: parsed.workbook.sheets.map((sheet: unknown) =>
      summaryModule.buildSheetSummary(sheet, { checked: true }),
    ),
  });
}
