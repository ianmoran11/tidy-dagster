...
def test_definitions_include_365_worksheet_cross_publication_batch() -> None:
    definitions = build_definitions(project_root=PROJECT)
    Definitions.validate_loadable(definitions)
    assert LARGE_BATCH_REGISTRY.worksheet_count == 365
    assert len(LARGE_BATCH_REGISTRY.entries) == 125
    assert len(LARGE_BATCH_ASSETS) == 125
    assert len(LARGE_BATCH_CHECKS) == 125
    assert len(LARGE_BATCH_JOBS) == 125
    assert definitions.metadata["product_prototype_large_batch_supported"].value
    assert definitions.metadata["product_prototype_large_batch_worksheets"].value == 365
    assert {
        spec.family_id
