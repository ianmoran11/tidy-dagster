...
def test_definitions_include_129_worksheet_cross_publication_batch() -> None:
    definitions = build_definitions(project_root=PROJECT)
    Definitions.validate_loadable(definitions)
    assert LARGE_BATCH_REGISTRY.worksheet_count == 129
    assert len(LARGE_BATCH_REGISTRY.entries) == 27
    assert len(LARGE_BATCH_ASSETS) == 27
    assert len(LARGE_BATCH_CHECKS) == 27
    assert len(LARGE_BATCH_JOBS) == 27
    assert definitions.metadata["product_prototype_large_batch_supported"].value
    assert definitions.metadata["product_prototype_large_batch_worksheets"].value == 129
    assert {
        spec.family_id
