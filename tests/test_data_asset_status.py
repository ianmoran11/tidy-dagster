...
def test_current_dashboard_reports_publication_grouped_clean_sheet_assets() -> None:
    ...
    assert len(status.cohorts) == 225
    assert len(status.assets) == 584
    assert status.physical_workbook_count == 84
    ...
    assert len({asset.csv_route for asset in status.assets}) == 584
    assert sum(asset.canonical_count or 0 for asset in status.assets) == 491403
    offenders = [
        asset
...
def test_html_is_single_file_safe_and_interactive_without_dependencies() -> None:
    ...
    assert rendered.count('class="asset-pair"') == 584
    assert rendered.count('class="detail-toggle"') == 584
    assert rendered.count('class="button csv-link"') == 584
    assert rendered.count("Open CSV") == 584
    ...
    assert rendered.count('class="coverage-row"') == 225
    assert rendered.count('class="coverage-cell coverage-complete"') == 584
    assert rendered.count('class="coverage-meter"') == 6
    assert rendered.count("<strong>584/584</strong>") == 6
    ...
    assert all(
        ...
        for value in (
            ...
            "Automated checks",
            "Flagged issues",
            "584 sheet-assets across 84 physical workbooks",
...
def test_coverage_matrix_keeps_multiple_assets_in_one_year_compact() -> None:
    ...
    assert rendered.count('class="coverage-row"') == 225
    assert rendered.count("data-target-year=") == 584
    assert (
    ...
    assert rendered.count("<strong>585/585</strong>") == 6
...
def test_coverage_matrix_distinguishes_not_registered_cells() -> None:
    ...
    assert rendered.count('class="coverage-cell coverage-absent"') == 344
    assert rendered.count('class="coverage-cell coverage-complete"') == 583
    assert "Not registered in this prototype scope" in rendered
    assert rendered.count("<strong>583/583</strong>") == 6
