...
def test_current_dashboard_reports_publication_grouped_clean_sheet_assets() -> None:
    ...
    assert [
        ...
        ("criminal-courts-australia", "fiscal-year", 18),
    ]
    assert len(status.cohorts) == 50
    assert len(status.assets) == 202
    ...
    assert len({asset.csv_route for asset in status.assets}) == 202
    assert sum(asset.canonical_count or 0 for asset in status.assets) == 163680
    offenders = [
        asset
    ...
    assert len(criminal_courts) == 48
    assert sum(asset.canonical_count or 0 for asset in criminal_courts) == 94380
    assert all(
...
def test_html_is_single_file_safe_and_interactive_without_dependencies() -> None:
    ...
    assert rendered.count('class="asset-pair"') == 202
    assert rendered.count('class="detail-toggle"') == 202
    assert rendered.count('class="button csv-link"') == 202
    assert rendered.count("Open CSV") == 202
    ...
    assert rendered.count('class="coverage-row"') == 50
    assert rendered.count('class="coverage-cell coverage-complete"') == 202
    assert rendered.count('class="coverage-meter"') == 6
    assert rendered.count("<strong>202/202</strong>") == 6
    ...
    assert all(
        ...
        for value in (
            ...
            "Automated checks",
            "Flagged issues",
            "202 sheet-assets across 27 physical workbooks",
...
def test_coverage_matrix_keeps_multiple_assets_in_one_year_compact() -> None:
    ...
    assert rendered.count('class="coverage-row"') == 50
    assert rendered.count("data-target-year=") == 202
    assert (
    ...
    assert rendered.count("<strong>203/203</strong>") == 6
...
def test_coverage_matrix_distinguishes_not_registered_cells() -> None:
    ...
    assert rendered.count('class="coverage-cell coverage-absent"') == 26
    assert rendered.count('class="coverage-cell coverage-complete"') == 201
    assert "Not registered in this prototype scope" in rendered
    assert rendered.count("<strong>201/201</strong>") == 6


def test_each_asset_csv_route_contains_only_that_assets_rows() -> None:
    status = build_dashboard(PROJECT)
    payloads = build_asset_csv_payloads(PROJECT, status)
    assert len(payloads) == 202
    observed_rows = 0
    for asset in status.assets:
