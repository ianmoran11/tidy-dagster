...
def test_current_dashboard_reports_publication_grouped_clean_sheet_assets() -> None:
    ...
    assert [
        ...
        ("criminal-courts-australia", "fiscal-year", 10),
    ]
    assert len(status.cohorts) == 42
    assert len(status.assets) == 183
    assert status.physical_workbook_count == 27
    ...
    assert len({asset.csv_route for asset in status.assets}) == 183
    assert sum(asset.canonical_count or 0 for asset in status.assets) == 138078
    offenders = [
        asset
    ...
    assert len(criminal_courts) == 29
    assert sum(asset.canonical_count or 0 for asset in criminal_courts) == 68778
    assert all(
...
def test_html_is_single_file_safe_and_interactive_without_dependencies() -> None:
    ...
    assert rendered.count('class="asset-pair"') == 183
    assert rendered.count('class="detail-toggle"') == 183
    assert rendered.count('class="button csv-link"') == 183
    assert rendered.count("Open CSV") == 183
    ...
    assert rendered.count('class="coverage-row"') == 42
    assert rendered.count('class="coverage-cell coverage-complete"') == 183
    assert rendered.count('class="coverage-meter"') == 6
    assert rendered.count("<strong>183/183</strong>") == 6
    ...
    assert all(
        ...
        for value in (
            ...
            "Automated checks",
            "Flagged issues",
            "183 sheet-assets across 27 physical workbooks",
...
def test_coverage_matrix_keeps_multiple_assets_in_one_year_compact() -> None:
    ...
    assert rendered.count('class="coverage-row"') == 42
    assert rendered.count("data-target-year=") == 183
    assert (
    ...
    assert rendered.count("<strong>184/184</strong>") == 6
...
def test_coverage_matrix_distinguishes_not_registered_cells() -> None:
    ...
    assert rendered.count('class="coverage-cell coverage-absent"') == 13
    assert rendered.count('class="coverage-cell coverage-complete"') == 182
    assert "Not registered in this prototype scope" in rendered
    assert rendered.count("<strong>182/182</strong>") == 6


def test_each_asset_csv_route_contains_only_that_assets_rows() -> None:
    status = build_dashboard(PROJECT)
    payloads = build_asset_csv_payloads(PROJECT, status)
    assert len(payloads) == 183
    observed_rows = 0
    for asset in status.assets:
