...
def test_current_dashboard_reports_publication_grouped_clean_sheet_assets() -> None:
    ...
    assert [
        ...
        ("criminal-courts-australia", "fiscal-year", 89),
    ]
    assert len(status.cohorts) == 121
    assert len(status.assets) == 368
    assert status.physical_workbook_count == 55
    ...
    assert len({asset.csv_route for asset in status.assets}) == 368
    assert sum(asset.canonical_count or 0 for asset in status.assets) == 314405
    offenders = [
        asset
    ...
    assert len(criminal_courts) == 214
    assert sum(asset.canonical_count or 0 for asset in criminal_courts) == 245105
    assert all(
...
def test_html_is_single_file_safe_and_interactive_without_dependencies() -> None:
    ...
    assert rendered.count('class="asset-pair"') == 368
    assert rendered.count('class="detail-toggle"') == 368
    assert rendered.count('class="button csv-link"') == 368
    assert rendered.count("Open CSV") == 368
    ...
    assert rendered.count('class="coverage-row"') == 121
    assert rendered.count('class="coverage-cell coverage-complete"') == 368
    assert rendered.count('class="coverage-meter"') == 6
    assert rendered.count("<strong>368/368</strong>") == 6
    ...
    assert all(
        ...
        for value in (
            ...
            "Automated checks",
            "Flagged issues",
            "368 sheet-assets across 55 physical workbooks",
...
def test_coverage_matrix_keeps_multiple_assets_in_one_year_compact() -> None:
    ...
    assert rendered.count('class="coverage-row"') == 121
    assert rendered.count("data-target-year=") == 368
    assert (
    ...
    assert rendered.count("<strong>369/369</strong>") == 6
...
def test_coverage_matrix_distinguishes_not_registered_cells() -> None:
    ...
    assert rendered.count('class="coverage-cell coverage-absent"') == 144
    assert rendered.count('class="coverage-cell coverage-complete"') == 367
    assert "Not registered in this prototype scope" in rendered
    assert rendered.count("<strong>367/367</strong>") == 6


def test_each_asset_csv_route_contains_only_that_assets_rows() -> None:
    status = build_dashboard(PROJECT)
    payloads = build_asset_csv_payloads(PROJECT, status)
    assert len(payloads) == 368
    observed_rows = 0
    for asset in status.assets:
